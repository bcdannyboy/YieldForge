...
3. Implement a paired known-only lower bound with exactly the same algorithm and compute contract, differing only in visibility.
4. Cross-check tiny cases against exhaustive enumeration and assert `lower_bound <= exact_optimum <= frozen_baseline_cost`. A violation is `invalid_test`, never economic evidence.
5. Run focused tests and commit.
