...
## Frozen pack construction

All selection uses source geometry, source demand, fixed identifiers, and hashes only. Candidate
width, density, residual differences, fit witnesses, replay costs, and M11 outcomes are forbidden
inputs.

The root generator seed is `2026082901`. Sub-seeds are the first unsigned 64 bits of
`sha256("yieldforge.m11-pack.v1|2026082901|" + label)`, where labels bind corpus, partition,
stream ordinal, and operation.

Lectra holdout families are connected components over shared catalog shape hashes among the 203
M3 tasks. The split key is `sha256("|".join(sorted_task_ids))`; a family is calibration when
`int(sha256("m11|lectra|family-split-v1|m11|" + split_key), 16) % 7 < 2`. This freezes 17
families / 58 tasks for calibration and 52 families / 145 tasks for confirmation.

LOCo execution preserves raw normalized geometry and isolates material compatibility by source
instance because coordinate scales are incomparable. Holdout grouping uses a separate identity
invariant to translation, uniform scale, and quarter turns but not reflection. It yields nine
connected source-instance components. The five singleton components `swim`, `dagli`, `albano`,
`marques`, and `mao` are calibration (146 source rows); every other component is confirmation
(365 rows). Repeated source rows remain repeated evidence.

Each partition contains four outcome-blind regimes:

- `recurrent`: four templates repeated six times;
- `mixed`: twelve templates repeated twice;
- `high_mix`: twenty-four distinct templates; and
- `regime_shift`: twelve events from one family subset followed by twelve from a disjoint subset.

Calibration has two streams per regime and confirmation has five. A Lectra event uses one complete
source task. A LOCo event uses eight distinct item rows from one source instance; generated quantity
is `1 + floor((source_demand - 1) / 25)`, capped by construction at four. LOCo stock and its
guaranteed fallback are generated by deterministic first-fit-decreasing bounding-box shelves over
eight registered target widths, choosing minimum envelope area, then width, then width index.

Streams start Monday `2026-01-05T08:00:00Z` and contain three releases per weekday at 08:00,
11:00, and 14:00 for eight weekdays. `known_at = released_at - 24 hours`. Due-time buckets are
four at 12 hours, eight at 24, eight at 48, and four at 72, hash-permuted. Six generated customers
occur `[6, 5, 4, 4, 3, 2]` times. Lectra has five assumed material keys occurring
`[8, 6, 4, 3, 3]` times; LOCo material keys include source instance.

Each confirmation stream has a no-signal twin that preserves geometry, chronology, quantities,
customers, due times, and candidates while deranging payload order and assigning a unique material
key per event. Each corpus also has three hard nulls (single action; unique materials plus single
action; all work known plus single action) and six preregistered three-event exact audits: two
central, two adverse, and two null.
...
Costs are normalized sheet-equivalent units, not dollars or observed factory rates. A reference
sheet costs `100.000000`. The frozen profiles are:

| Term | Optimistic | Central | Fixed adverse |
| --- | ---: | ---: | ---: |
| Virgin material / reference-sheet area | 100.000000 | 100.000000 | 100.000000 |
| Scrap and terminal credit / reference-sheet area | 0 | 10.000000 | 25.000000 |
| Return handling / eligible remnant | 0 | 0.250000 | 1.000000 |
| Retrieval handling / remnant use | 0 | 0.250000 | 1.000000 |
| Storage / reference-sheet area / 30 days | 0 | 0.500000 | 2.000000 |

All rates are `assumed`. Higher adverse scrap credit represents a stronger incumbent liquidation
alternative; higher handling and storage represent greater operational friction. Labor, energy,
tool wear, setup, lateness, financing, tax, overhead, and stockout costs remain excluded by the M0
scope.

Lectra reference area is the derived median M3/M4 source-sheet area,
`1,296,449,632.0`. LOCo uses the median verified generated fallback-stock area within each source
instance. The optimistic Gate 1 lower bound uses empty initial inventory, fractional stock,
perfectly fungible compatible material, zero process loss/friction, and downward six-decimal
rounding. Realized feasible costs retain six-decimal half-up ledger rounding.
