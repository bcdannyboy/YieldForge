...
M7 is next. It must define a batch-capable replay action boundary, generate shared verified
candidate archives, compare competent as-of-safe baseline variants only on calibration streams,
and freeze the winning deterministic policy before any evaluation result is inspected.
